...
``` r
### load.packages isn't the right way to load an installed package. Should be library() with the package name inside ### 
...
``` r
### Although dimensions are still returned, the instructions call for the dplyr version of dim(), which is glimpse() ###
...
``` r
### Condition for line 3 should only specify years > 2000; replace "month == 2000" with year in line 55 since we are looking for filming in the year 2000 ###
...
``` r
### Instead of listing all column names individually, use the more general everything() function to keep the rest of the data amenable to adding/deleting columns ###
...
``` r
### Only need to use one = sign to specify change in name! ###
movieLens <- movieLens %>%
...
``` r
### transmute() adds the new variable and drops the existing ones ### 
transmute(movieLens,
...
``` r
### Instead of using group_by() with tally() use the short-hand function, count() ###
...
``` r
### Don't need to list title and rating within the c() vector function ###
...
``` r
### mutate() evaluates each user's rating individually whereas summarise() considers all users. Also need to group by title first so the summarise function can evaluate each movie's stats ###
...
``` r
### Needed to specify the height and mass within a vector and without quotations (being variable names) ###
#starWars %>%
 # group_by(species) %>%
...
``` r
### Was missing a comma after ~birth_location and needed to specify location names in quotes ###
